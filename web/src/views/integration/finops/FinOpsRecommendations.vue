...
function recommendationName(row) {
  const accountName = accounts.value.find(item => String(item.id) === String(row.analysisAccountId))?.name || (Number(row.analysisAccountId) ? `云账号 #${row.analysisAccountId}` : '全部云账号')
  const month = row.analysisMonth || '未标注账期'
  return `${accountName}｜${month}｜${strategyLabel(row)}优化建议`
}
  ...
  <div class="finops-page">
    ...
    <section class="finops-panel" v-loading="loading">
      <div class="finops-head">
        <div><h2>优化建议</h2><p>命名规范：云账号｜分析账期｜分析策略。未配置 AI 时自动使用内置 FinOps 策略。</p></div>
        <div class="finops-actions">
          <el-select v-model="analysisAccountId" clearable placeholder="全部云账号" style="width: 170px" @change="load">
            <el-option v-for="item in accounts" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
          <el-date-picker v-model="analysisMonth" type="month" value-format="YYYY-MM" format="YYYY-MM" clearable placeholder="分析账期" style="width: 145px" @change="load" />
          <el-button type="primary" @click="openGenerateDialog">生成建议</el-button>
        </div>
      </div>
      <el-table :data="rows" stripe>
        <el-table-column label="优化建议报告" min-width="300"><template #default="{ row }">{{ recommendationName(row) }}</template></el-table-column>
