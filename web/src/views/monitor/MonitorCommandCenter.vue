  ...
  <div ref="screenRef" class="command-center" :class="{ loading }">
    ...
    <main class="cc-layout">
      ...
      <section class="cc-center">
        ...
        <div class="hot-row"><article v-for="title in hotPanelTitles" :key="title" class="cc-panel hot-panel"><div class="panel-heading"><span>▣ {{ title }}</span><em>{{ mt('topFive') }}</em></div><div v-if="data.hotHosts?.length" class="hot-host"><div v-for="host in data.hotHosts.slice(0, 3)" :key="host.name"><span>{{ host.name }}</span><b :class="host.aliveStatus === 1 ? 'ok-text' : 'danger-text'">{{ host.aliveStatus === 1 ? mt('activeShort') : mt('needCheckShort') }}</b></div></div><div v-else class="panel-empty">{{ mt('noMonitoringData') }}</div></article></div>
      </section>
