...
const hotPanelTitles = computed(() => [mt('cpuHotspot'), mt('memoryHotspot'), mt('diskHotspot')])
...
<template>
  <div ref="screenRef" class="command-center" :class="{ loading }">
    <header class="cc-header">
      <div class="cc-system"><i /> {{ mt('systemOnline') }} <span>{{ mt('dataRefreshedAt', { time: timeText(data.refreshedAt || now) }) }}</span></div>
      <div class="cc-title"><small>{{ mt('commandCenterEyebrow') }}</small><strong>{{ mt('cockpitTitle') }}</strong></div>
      <div class="cc-tools"><b>{{ timeText() }}</b><span>{{ dateText() }}</span><div class="cc-auto" :class="{ enabled: autoRefresh }"><i /><span>{{ mt('autoRefreshLabel') }}</span><b>{{ mt('thirtySeconds') }}</b><el-switch v-model="autoRefresh" size="small" @change="syncTimers" /></div><el-button circle :icon="Refresh" :loading="loading" @click="loadData" /><el-button circle :icon="FullScreen" @click="toggleFullscreen" /></div>
    </header>

    <main class="cc-layout">
      <section class="cc-column left-column">
        <article class="cc-panel alerts-panel"><div class="panel-heading"><span>{{ mt('realtimeAlerts') }}</span><router-link to="/monitor/alert-events">{{ mt('viewAll') }} {{ number(overview.firingCount) }}</router-link></div><div class="alert-list"><div v-for="item in data.recentAlerts" :key="item.id" class="alert-item"><el-tag size="small" :type="severityType(item.severity)">{{ item.severity || 'P3' }}</el-tag><div><b>{{ item.ruleName || mt('unnamedAlert') }}</b><p>{{ item.summary || item.metric || mt('awaitingHandle') }}</p></div></div><div v-if="!data.recentAlerts?.length" class="panel-empty">{{ mt('noActiveAlerts') }}</div></div></article>
        <article class="cc-panel trend-panel"><div class="panel-heading"><span>{{ mt('todayTrend') }}</span><em>{{ mt('trendFiringRecovery') }}</em></div><div class="mini-bars"><div v-for="item in alertTrend" :key="item.date" class="mini-bar"><div class="bar-stack"><i :style="{ height: `${Math.max(3, Number(item.triggered || 0) / trendMax * 100)}%` }" /><b :style="{ height: `${Math.max(3, Number(item.recovered || 0) / trendMax * 100)}%` }" /></div><small>{{ item.date }}</small></div><div v-if="!alertTrend.length" class="panel-empty">{{ mt('noTrendData') }}</div></div></article>
        <article class="cc-panel rule-panel"><div class="panel-heading"><span>▣ Alert Rule TOP5</span><em>{{ mt('activeEvents') }}</em></div><div class="rank-list"><div v-for="(item, index) in data.topRules" :key="item.name"><b>0{{ index + 1 }}</b><span>{{ item.name }}</span><strong>{{ number(item.count) }}</strong></div><div v-if="!data.topRules?.length" class="panel-empty">{{ mt('noRankData') }}</div></div></article>
      </section>

      <section class="cc-center">
        <div class="metric-strip"><div class="metric-card"><small>{{ mt('assetTotal') }}</small><strong>{{ number(asset.total) }}</strong><span>{{ mt('managedAssets') }}</span></div><div class="metric-card danger"><small>{{ mt('activeAlert') }}</small><strong>{{ number(overview.firingCount) }}</strong><span>{{ mt('pendingEvents') }}</span></div><div class="metric-card"><small>{{ mt('onlineHosts') }}</small><strong>{{ number(asset.onlineHosts) }} / {{ number(asset.hosts) }}</strong><span>{{ mt('hostSurvivalRate') }}</span></div><div class="metric-card green"><small>{{ mt('monitoringCoverage') }}</small><strong>{{ percent(onlineRate) }}</strong><span>{{ mt('hostOnlineBasis') }}</span></div><div class="metric-card amber"><small>K8s Node</small><strong>{{ number(asset.clusters) }}</strong><span>{{ mt('linkedClusters') }}</span></div></div>
        <article class="globe-panel"><div class="globe-heading"><div><small>{{ mt('resourceSituation') }}</small><h2>{{ mt('resourceSpaceTitle') }}</h2><p>{{ mt('globeHint') }}</p></div><button type="button" @click="resetGlobe"><el-icon><RefreshLeft /></el-icon>{{ mt('resetView') }}</button></div><canvas ref="globeRef" class="globe-canvas" @pointerdown="onPointerDown" @pointermove="onPointerMove" @pointerup="onPointerUp" @pointerleave="onPointerUp" /><div class="globe-stats left"><span>{{ mt('physicalHost') }}</span><b>{{ number(asset.hosts) }}</b><small>{{ mt('physicalCloudHost') }}</small><span>{{ mt('monitoring') }}</span><b>{{ percent(onlineRate) }}</b><small>{{ mt('onlineCoverage') }}</small></div><div class="globe-stats right"><span>{{ mt('activeAlert') }}</span><b class="danger-text">{{ number(overview.firingCount) }}</b><small>{{ mt('activeAlert') }}</small><span>{{ mt('authRate') }}</span><b>{{ percent(onlineRate) }}</b><small>{{ mt('assetAuthRate') }}</small></div><footer><b>{{ number(asset.total) }}</b><span>{{ mt('managedAssetsShort') }}</span><small>{{ mt('globeFooterHint') }}</small></footer></article>
        <div class="hot-row"><article v-for="title in hotPanelTitles" :key="title" class="cc-panel hot-panel"><div class="panel-heading"><span>▣ {{ title }}</span><em>{{ mt('topFive') }}</em></div><div v-if="data.hotHosts?.length" class="hot-host"><div v-for="host in data.hotHosts.slice(0, 3)" :key="host.name"><span>{{ host.name }}</span><b :class="host.aliveStatus === 1 ? 'ok-text' : 'danger-text'">{{ host.aliveStatus === 1 ? mt('hostOnlineShort') : mt('needCheckShort') }}</b></div></div><div v-else class="panel-empty">{{ mt('noMonitoringData') }}</div></article></div>
      </section>

      <section class="cc-column right-column">
        <article class="cc-panel composition-panel"><div class="panel-heading"><span>{{ mt('assetBrands') }}</span><em>{{ mt('topCount', { count: data.resourceComposition?.length || 0 }) }}</em></div><div class="composition-list"><div v-for="item in data.resourceComposition" :key="item.name"><span>{{ item.name }}</span><i><b :style="{ width: `${asset.total ? Number(item.count || 0) / asset.total * 100 : 0}%` }" /></i><strong>{{ number(item.count) }}</strong></div></div></article>
        <article class="cc-panel resource-panel"><div class="panel-heading"><span>{{ mt('resourceOverview') }}</span><em>{{ mt('totalCountSuffix', { count: number(asset.total) }) }}</em></div><div class="resource-total"><div v-for="item in data.resourceComposition" :key="item.name"><span>{{ item.name }}</span><b>{{ number(item.count) }}</b></div></div><div class="coverage-ring"><strong>{{ percent(onlineRate) }}</strong><span>{{ mt('onlineCoverage') }}</span></div></article>
        <article class="cc-panel region-panel"><div class="panel-heading"><span>{{ mt('dcAssetDistribution') }}</span><em>{{ mt('regionCount', { count: data.regions?.length || 0 }) }}</em></div><div class="region-list"><div v-for="item in data.regions" :key="item.name"><i>◇</i><span>{{ item.name }}</span><b>{{ number(item.count) }}</b></div><div v-if="!data.regions?.length" class="panel-empty">{{ mt('noRegionData') }}</div></div></article>
        <div class="deadline"><span>◷ {{ mt('pendingAlertsDeadline') }} <b>{{ number(overview.unclaimedCount) }}</b></span><span>● {{ mt('highPriority') }} <b>{{ number(overview.criticalCount) }}</b></span></div>
      </section>
    </main>
  </div>
</template>
