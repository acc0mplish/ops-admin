  ...
  <div ref="screenRef" class="command-center" :class="{ loading }">
    ...
    <main class="cc-layout">
      <section class="cc-column left-column">
        <article class="cc-panel alerts-panel"><div class="panel-heading"><span>▣ 实时告警</span><router-link to="/monitor/alert-events">查看全部 {{ number(overview.firingCount) }}</router-link></div><div class="alert-list"><div v-for="item in data.recentAlerts" :key="item.id" class="alert-item"><el-tag size="small" :type="severityType(item.severity)">{{ item.severity || 'P3' }}</el-tag><div><b>{{ item.ruleName || '未命名告警' }}</b><p>{{ item.summary || item.metric || '等待处理' }}</p></div></div><div v-if="!data.recentAlerts?.length" class="panel-empty">◇ 当前没有活跃告警</div></div></article>
        <article class="cc-panel trend-panel"><div class="panel-heading"><span>▣ 今日告警趋势</span><em>触发 / 恢复</em></div><div class="mini-bars"><div v-for="item in alertTrend" :key="item.date" class="mini-bar"><div class="bar-stack"><i :style="{ height: `${Math.max(3, Number(item.triggered || 0) / trendMax * 100)}%` }" /><b :style="{ height: `${Math.max(3, Number(item.recovered || 0) / trendMax * 100)}%` }" /></div><small>{{ item.date }}</small></div><div v-if="!alertTrend.length" class="panel-empty">◇ 暂无今日趋势数据</div></div></article>
        <article class="cc-panel rule-panel"><div class="panel-heading"><span>▣ 告警规则 TOP5</span><em>活跃事件</em></div><div class="rank-list"><div v-for="(item, index) in data.topRules" :key="item.name"><b>0{{ index + 1 }}</b><span>{{ item.name }}</span><strong>{{ number(item.count) }}</strong></div><div v-if="!data.topRules?.length" class="panel-empty">◇ 暂无排行数据</div></div></article>
      </section>
