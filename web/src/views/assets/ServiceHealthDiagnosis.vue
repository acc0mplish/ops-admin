...
const flamePreviewUrl = ref('')

function clearFlamePreview() {
  if (flamePreviewUrl.value) URL.revokeObjectURL(flamePreviewUrl.value)
  flamePreviewUrl.value = ''
}

watch(() => diagnostic.value?.flameHtml, (html) => {
  clearFlamePreview()
  if (html) flamePreviewUrl.value = URL.createObjectURL(new Blob([html], { type: 'text/html;charset=utf-8' }))
})
...
function reset(level) { if (level === 'service') target.value.workloadKey = ''; if (level === 'service' || level === 'workload') { target.value.podName = ''; runtime.value = {} }; if (level !== 'pod') containers.value = []; target.value.container = ''; target.value.pid = ''; processes.value = []; environment.value = null; diagnostic.value = null }
async function refreshProcesses() { if (!canInspect.value) return ElMessage.warning('请先选择服务、工作负载、Pod 和容器'); actionLoading.value = true; try { const data = await queryAssetServiceDiagnosisProcesses(params.value); processes.value = data.processes || []; target.value.pid = ''; environment.value = await queryAssetServiceDiagnosisEnvironment(params.value); ElMessage.success(`已加载 ${processes.value.length} 个 Java 进程，并完成环境检查`) } finally { actionLoading.value = false } }
...
async function runDiagnostic(operation, extra = {}) { if (!target.value.pid) return ElMessage.warning('请先刷新并选择 Java 进程 PID'); if (!environment.value?.ready) return ElMessage.warning('请先检查 Arthas 环境'); diagnosticLoading.value = true; diagnostic.value = null; try { const result = await queryAssetServiceDiagnosisRun({ ...params.value, operation, ...extra }); diagnostic.value = result.data } finally { diagnosticLoading.value = false } }
function changeTab(tab) { activeTab.value = tab; diagnostic.value = null }
onMounted(loadServices)
onBeforeUnmount(clearFlamePreview)
</script>
  ...
  <div class="diagnosis-page" v-loading="loading">
    ...
    <section class="target-card">
      <div class="target-fields">
        ...
        <label>容器<el-select v-model="target.container" :disabled="!target.podName" placeholder="选择容器"><el-option v-for="item in containers" :key="item" :label="item" :value="item"/></el-select></label>
        <label>进程<el-select v-model="target.pid" :disabled="!processes.length" placeholder="刷新进程后选择"><el-option v-for="item in processes" :key="item.pid" :label="`PID ${item.pid} · ${item.name}`" :value="item.pid"/></el-select></label>
      </div>
    ...
    <section class="diagnostic-card">
      ...
      <div v-else class="result-panel">
        ...
        <template v-if="activeTab === 'flame' && diagnostic.flameHtml">
          <el-alert type="success" :closable="false" show-icon :title="`墙钟火焰图已生成，共采集 ${diagnostic.samples || '-'} 个样本，可直接在页面中交互查看`" />
          <iframe class="flame-preview" :src="flamePreviewUrl" sandbox="allow-scripts" title="Arthas 火焰图预览" />
        </template>
