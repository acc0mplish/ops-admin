...
| P1 | 取消默认管理员密码与登录页预填 | 首次初始化要求 `OPS_ADMIN_INITIAL_PASSWORD`（至少 12 位）；浏览器登录页两个输入框均为空 | 通过 |
...
- 生产部署必须在 `deploy/.env` 配置唯一且至少 12 位的 `OPS_ADMIN_INITIAL_PASSWORD`，可选配置 `OPS_ADMIN_INITIAL_USERNAME`。
