...
func (s *Service) GetAssetDatabaseMetrics(id uint, rangeKey, startValue, endValue string) (map[string]any, error) {
	...
	response := map[string]any{
		"enabled": item.MonitorEnabled,
		"range":   map[string]any{"key": normalizedRange, "startAt": startAt, "endAt": endAt},
	...
	if sample, collectErr := s.collectAssetDatabaseMetrics(item); collectErr == nil {
		if err := s.db.Create(&sample).Error; err != nil {
			response["collectError"] = err.Error()
		}
	} else {
		response["collectError"] = collectErr.Error()
	}

	endAt = time.Now()
	var snapshots []model.AssetDatabaseMetricSnapshot
	if err := s.db.Where("database_id = ? AND created_at >= ? AND created_at <= ?", id, startAt, endAt).
		Order("created_at ASC").Find(&snapshots).Error; err != nil {
		return nil, err
	}
...
func emptyDatabaseMetricSeries() map[string]any {
	...
	for _, key := range databaseMetricKeys() {
		metrics[key] = map[string]any{"latest": nil, "points": []map[string]any{}}
	}
	return metrics
}

func databaseMetricKeys() []string {
	return []string{"qps", "connections", "threads", "bytesIn", "bytesOut", "slowQueries", "uptime", "cacheHitRate"}
}
...
func databaseMetricSeries(snapshots []model.AssetDatabaseMetricSnapshot) map[string]any {
	...
	for _, key := range databaseMetricKeys() {
		...
		for _, snapshot := range snapshots {
			if value, ok := snapshot.Metrics[key]; ok && !math.IsNaN(value) && !math.IsInf(value, 0) {
				points = append(points, map[string]any{"timestamp": snapshot.CreatedAt.UnixMilli(), "value": value})
			}
		}
		var latest any
		if len(points) > 0 {
...
func (s *Service) collectAssetDatabaseMetrics(item *model.AssetDatabase) (model.AssetDatabaseMetricSnapshot, error) {
	...
	if err != nil {
		return model.AssetDatabaseMetricSnapshot{}, err
	}
	...
	// Retain source counters alongside the rendered values so the next sample
	// can calculate per-second rates without an external time-series system.
	for _, key := range []string{"counterQueries", "counterBytesIn", "counterBytesOut", "counterSlowQueries"} {
...
func deriveDatabaseMetrics(current, previous map[string]float64, elapsed time.Duration) map[string]float64 {
	...
	for _, key := range []string{"connections", "threads", "uptime", "cacheHitRate"} {
		metrics[key] = current[key]
	}
	seconds := elapsed.Seconds()
	if previous == nil || seconds <= 0 || seconds > 24*time.Hour.Seconds() {
		metrics["qps"], metrics["bytesIn"], metrics["bytesOut"], metrics["slowQueries"] = 0, 0, 0, 0
		return metrics
	}
...
func counterRate(current, previous, seconds float64) float64 {
...
func (s *Service) readAssetDatabaseMetricCounters(item model.AssetDatabase) (map[string]float64, error) {
	...
	case "mysql":
		return s.readMySQLDatabaseMetrics(item)
	case "postgresql":
		return s.readPostgresDatabaseMetrics(item)
	case "mongodb":
		return s.readMongoDatabaseMetrics(item)
	case "redis":
		return s.readRedisDatabaseMetrics(item)
	default:
		return nil, fmt.Errorf("数据库类型 %s 暂不支持监控仪表盘", item.DBType)
	}
}
...
func (s *Service) readMySQLDatabaseMetrics(item model.AssetDatabase) (map[string]float64, error) {
	...
	defer cleanup()
	rows, err := db.Query(`SHOW GLOBAL STATUS WHERE Variable_name IN ('Queries','Threads_connected','Threads_running','Bytes_received','Bytes_sent','Slow_queries','Uptime','Innodb_buffer_pool_reads','Innodb_buffer_pool_read_requests')`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		values[strings.ToLower(name)], _ = strconv.ParseFloat(value, 64)
	}
	reads, requests := metricValue(values, "Innodb_buffer_pool_reads"), metricValue(values, "Innodb_buffer_pool_read_requests")
	hitRate := 0.0
	if requests > 0 {
		hitRate = math.Max(0, 100*(1-reads/requests))
	}
	return map[string]float64{
		"counterQueries": metricValue(values, "Queries"), "connections": metricValue(values, "Threads_connected"), "threads": metricValue(values, "Threads_running"),
		"counterBytesIn": metricValue(values, "Bytes_received"), "counterBytesOut": metricValue(values, "Bytes_sent"), "counterSlowQueries": metricValue(values, "Slow_queries"),
		"uptime": metricValue(values, "Uptime"), "cacheHitRate": hitRate,
	}, rows.Err()
}
...
func (s *Service) readPostgresDatabaseMetrics(item model.AssetDatabase) (map[string]float64, error) {
	...
	if err := row.Scan(&queries, &connections, &operations, &hit, &read, &uptime); err != nil {
		return nil, err
	}
	hitRate := 0.0
	if hit+read > 0 {
		hitRate = 100 * hit / (hit + read)
	}
...
func (s *Service) readMongoDatabaseMetrics(item model.AssetDatabase) (map[string]float64, error) {
	...
	client, err := mongoConnectWithAuth(ctx, uri, target.Username, target.Password)
	if err != nil {
		return nil, err
	}
	...
	hitRate := 0.0
	hits, misses := get("wiredTiger", "cache", "pages read into cache"), get("wiredTiger", "cache", "pages requested from the cache")
	if hits+misses > 0 {
		hitRate = 100 * misses / (hits + misses)
	}
...
func mongoConnectWithAuth(ctx context.Context, uri, username, password string) (*mongo.Client, error) {
...
func (s *Service) readRedisDatabaseMetrics(item model.AssetDatabase) (map[string]float64, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 8*time.Second)
	defer cancel()
	index, _ := strconv.Atoi(strings.TrimSpace(target.DBName))
	client := redis.NewClient(&redis.Options{Addr: net.JoinHostPort(target.Host, strconv.Itoa(databasePortByType("redis", target.Port))), Username: target.Username, Password: target.Password, DB: index})
	defer client.Close()
	values := map[string]float64{}
	// Redis INFO accepts at most one section per command. Query the sections
	// individually so Redis 6/7 and compatible implementations all work.
	for _, section := range []string{"stats", "clients", "server", "memory"} {
		...
		for _, line := range strings.Split(info, "\n") {
			...
			if len(pair) == 2 {
				values[pair[0]], _ = strconv.ParseFloat(pair[1], 64)
			}
		}
	}
